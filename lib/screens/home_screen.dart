...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  ...
  /// a frozen, feedback-free screen. The barrier also blocks a second tap on
  /// the menu while the first operation is still writing.
    ...
    try {
      switch (action) {
        ...
        case 'delete_all':
          ...
          // seven blobs, and the menu must not accept a second action
          // mid-write.
        ...
        actions: [
          ...
          MenuAnchor(
            menuChildren: [
              ...
              const Divider(),
              SubmenuButton(
                // Export sends data out of the app, so it takes the upload
                // icon; import takes download. These were the wrong way round.
                leadingIcon: const Icon(Icons.file_upload_outlined),
                menuChildren: [
                  MenuItemButton(
                    leadingIcon: const Icon(Icons.data_object),
                    onPressed: () => _handleBackupAction('export_json'),
                    child: const Text('Backup (JSON)'),
                  ),
                  MenuItemButton(
                    leadingIcon: const Icon(Icons.table_chart_outlined),
                    onPressed: () => _handleBackupAction('export_csv'),
                    child: const Text('Transactions (CSV)'),
                  ),
                  MenuItemButton(
                    leadingIcon: const Icon(Icons.picture_as_pdf_outlined),
                    onPressed: () => _handleBackupAction('export_pdf'),
              ...
              SubmenuButton(
                ...
                menuChildren: [
                  MenuItemButton(
                    leadingIcon: const Icon(Icons.data_object),
                    onPressed: () => _handleBackupAction('import_json'),
                    child: const Text('Backup (JSON)'),
                  ),
                  MenuItemButton(
                    // Content icon, matching the Export submenu's CSV row —
                    // a direction arrow here would fight the parent's.
                    leadingIcon: const Icon(Icons.table_chart_outlined),
                    onPressed: () => _handleBackupAction('import_csv'),
                    child: const Text('Transactions (CSV)'),
                  ),
                  MenuItemButton(
                    leadingIcon: const Icon(Icons.cloud_download_outlined),
                    onPressed: () => _handleBackupAction('import_drive'),
                    child: const Text('From Google Drive'),
                  ),
                ],
                child: const Text('Import'),
              ),
              const Divider(),
              // Error-tinted: a destructive action shouldn't look identical
              // to Import/Export one divider above it.
              MenuItemButton(
                leadingIcon: Icon(
                  Icons.delete_forever_outlined,
                  color: Theme.of(context).colorScheme.error,
                ),
                onPressed: () => _handleBackupAction('delete_all'),
                child: Text(
                  'Delete all data',
